...
export function calculateRowAndColumnMeta(pxtable: PxTable): columnRowMeta {
  ...
  for (const headingVariable of pxtable.heading) {
    columnCount *= headingVariable.values.length;
  }

  for (const stubVariable of pxtable.stub) {
    rowCount *= stubVariable.values.length;
  }
