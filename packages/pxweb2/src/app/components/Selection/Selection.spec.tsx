...
describe('Selection', () => {
  ...
  describe('handleCodeListChange', () => {
    ...
    const mockGetSelectedCodelists = vi.spyOn(selectionUtils, 'getSelectedCodelists');
    const mockUpdateSelectedCodelistForVariable = vi.spyOn(selectionUtils, 'updateSelectedCodelistForVariable');

    // Minimal context for handleCodeListChange
    function getContext({
      pxTableMetaToRender = { variables: [{ id: 'var1', codeLists: [{ id: 'codelist1' }] }], pathElements: [] } as { variables: Array<{ id: string; codeLists?: Array<{ id: string }> }>; pathElements: unknown[] } | null,
      selectedVBValues = [{ id: 'var1', selectedCodeList: 'codelist0', values: [] as string[] }],
      selectedTabId = 'tab1',
      i18n = { resolvedLanguage: 'en', dir: () => 'ltr' } as { resolvedLanguage?: string; dir: () => string },
      selectedItem = { value: 'codelist1' },
      varId = 'var1',
      updateSelectedCodelistForVariableReturn = [{ id: 'var1', selectedCodeList: 'codelist1', values: [] as string[] }],
    }: {
      pxTableMetaToRender?: { variables: Array<{ id: string; codeLists?: Array<{ id: string }> }>; pathElements: unknown[] } | null;
      selectedVBValues?: Array<{ id: string; selectedCodeList?: string; values: string[] }>;
      selectedTabId?: string;
      i18n?: { resolvedLanguage?: string; dir: () => string };
      selectedItem?: { value: string };
      varId?: string;
      updateSelectedCodelistForVariableReturn?: Array<{ id: string; selectedCodeList: string; values: string[] }>;
    } = {}) {
      // Mock TablesService
      const TablesService = {
        getMetadataById: vi.fn().mockResolvedValue({
          ...
          pathElements: [],
          metadata: { variables: [{ id: 'var1', codeLists: [{ id: 'codelist1' }] }], pathElements: [] },
        }),
      };
      const mapJsonStat2Response = vi.fn().mockReturnValue({ metadata: { variables: [{ id: 'var1', codeLists: [{ id: 'codelist1' }] }], pathElements: [] } });
      mockGetSelectedCodelists.mockReturnValue({ var1: 'codelist1' });
      mockUpdateSelectedCodelistForVariable.mockReturnValue(updateSelectedCodelistForVariableReturn);
      return {
        TablesService,
    ...
    it('should do nothing if language is undefined', async () => {
      const ctx = getContext({ i18n: { resolvedLanguage: undefined, dir: () => 'ltr' } });
    ...
    it('should do nothing if currentVariableMetadata is undefined', async () => {
      const ctx = getContext({ pxTableMetaToRender: { variables: [], pathElements: [] } });
      expect(ctx.pxTableMetaToRender?.variables.length).toBe(0);
    });

    it('should do nothing if codelist is not new', async () => {
      const ctx = getContext({
        selectedVBValues: [{ id: 'var1', selectedCodeList: 'codelist1', values: [] }],
        selectedItem: { value: 'codelist1' },
      });
    ...
    it('should handle API error gracefully', async () => {
      const ctx = getContext();
      ctx.TablesService.getMetadataById.mockRejectedValueOnce(new Error('API error'));
      expect.assertions(1);
      try {
